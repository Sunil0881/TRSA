...
const AchievementsInternational = () => {
    ...
    const fetchAchievements = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          // Filter only national level achievements
          const internationalAchievements = data.filter(
            (achievement) => achievement.level.toLowerCase() === "international"
          );
          setAchievements(internationalAchievements);
        } else {
      ...
      <div className="flex flex-col lg:flex-row gap-2">
        ...
        <div className="w-full lg:w-2/3 lg:max-h-screen lg:overflow-y-auto lg:pr-4 custom-scrollbar">
          {loading ? (
            ...
            <p>No international level achievements found</p>
          )}
        </div>
...
export default AchievementsInternational;
